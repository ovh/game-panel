...
import { AlertTriangle, Check, Info, Loader2, Plus, Save, Trash2 } from 'lucide-react';
...
export interface MinecraftSectionsProps {
  ...
  /** Whether the caller holds `server.env` on this server. The version /
   *  server-type picker is backed by env vars, which the backend redacts and
   *  ignores for callers without this permission — so the picker is hidden
...
function PropertiesSection({
  serverId, canRead, canWrite, borderColor, contentBg, textPrimary, textSecondary,
}: {
  serverId: number; canRead: boolean; canWrite: boolean;
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiClient.getMinecraftSettings(serverId);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await apiClient.patchMinecraftSettings(serverId, changed);
            ...
            {fields.map((field) => (
              <div key={field.key} className={`rounded-lg border ${borderColor} bg-gp-surface-base/45 p-3 sm:p-4 flex flex-col justify-center`}>
                <div className="grid grid-cols-1 sm:grid-cols-[minmax(185px,1.15fr)_minmax(0,1.1fr)] items-center gap-3 sm:gap-4 w-full">
                  <div className="relative" data-mc-help>
...
function ServerVersionSection({
  ...
  useEffect(() => {
    ...
    apiClient.getServer(serverId).then((server: any) => {
      if (cancelled) return;
      // DTO now returns `env` as an object; keep array/string handling for safety.
...
export function MinecraftSections({
  ...
  // The version / server-type picker is env-backed, so it is only available to
  // callers holding `server.env`.
    ...
    <div>
      ...
      {visited.has('settings') && (canReadSettings || canShowVersion) && (
        <div className={`space-y-4 ${activeTab !== 'settings' ? 'hidden' : ''}`}>
          {canReadSettings && (
            <PropertiesSection {...sectionProps} canRead={canReadSettings} canWrite={canWriteSettings} />
          )}
          {canShowVersion && (
            <ServerVersionSection
              serverId={serverId}
